...
; About:
;   Gets a ptr of first occurance of char c in string str, not more than n bytes
; Args:
;   const void* str - string ptr, null-terminator isn't required
;   char c - char to find
;   uint16_t size - intterate not more than n bytes of string
; Return:
;   const char* pos - ptr to the first occurance of char in str,
;       NULL if not found
; C Prototype:
;   const void* memchr(unsigned char* str, char c, uint16_t size);
memchr:
    push af  ; storing af
    push hl  ; storing hl
    push ix  ; storing ix
