...
;   None
; C Prototype:
;   void strappend(unsigned char* dst, uint16_t position, unsigned char c);
strappend:
    push af  ; storing af
    push hl  ; storing hl
    push bc  ; storing bc
    ...
    append_to_string_loop:
        ...
        jr z, append_to_string_loop_end  ; if current address == str start address
        dec hl  ; irrerating counter-clockwise
        jr append_to_string_loop
