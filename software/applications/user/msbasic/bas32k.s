...
; GENERAL EQUATES
        ...
        .equ    CTRLQ   ,   0x11         ; Control "Q"
        .equ    CTRLR   ,   0x12         ; Control "R"
        .equ    CTRLS   ,   0x13         ; Control "S"
        .equ    CTRLU   ,   0x15         ; Control "U"
        .equ    ESC     ,   0x1B         ; Escape
        .equ    DEL     ,   0x7F         ; Delete

; BASIC WORK SPACE LOCATIONS
; WRKSPC is defined in msbasic.s as a .bss symbol; address resolved by linker.

        .equ    USR     ,   WRKSPC+0x003 ; "USR (x)" jump
        .equ    OUTSUB  ,   WRKSPC+0x006 ; "OUT p,n"
        .equ    OTPORT  ,   WRKSPC+0x007 ; Port (p)
        .equ    DIVSUP  ,   WRKSPC+0x009 ; Division support routine
        .equ    DIV1    ,   WRKSPC+0x00A ; <- Values
        .equ    DIV2    ,   WRKSPC+0x00E ; <-   to
        .equ    DIV3    ,   WRKSPC+0x012 ; <-   be
        .equ    DIV4    ,   WRKSPC+0x015 ; <-inserted
        .equ    SEED    ,   WRKSPC+0x017 ; Random number seed
        .equ    LSTRND  ,   WRKSPC+0x03A ; Last random number
        .equ    INPSUB  ,   WRKSPC+0x03E ; #INP (x)" Routine
        .equ    INPORT  ,   WRKSPC+0x03F ; PORT (x)
        .equ    NULLS   ,   WRKSPC+0x041 ; Number of nulls
        .equ    LWIDTH  ,   WRKSPC+0x042 ; Terminal width
        .equ    COMMAN  ,   WRKSPC+0x043 ; Width for commas
        .equ    NULFLG  ,   WRKSPC+0x044 ; Null after input byte flag
        .equ    CTLOFG  ,   WRKSPC+0x045 ; Control "O" flag
        .equ    LINESC  ,   WRKSPC+0x046 ; Lines counter
        .equ    LINESN  ,   WRKSPC+0x048 ; Lines number
        .equ    CHKSUM  ,   WRKSPC+0x04A ; Array load/save check sum
        .equ    NMIFLG  ,   WRKSPC+0x04C ; Flag for NMI break routine
        .equ    BRKFLG  ,   WRKSPC+0x04D ; Break flag
        .equ    RINPUT  ,   WRKSPC+0x04E ; Input reflection
        .equ    STRSPC  ,   WRKSPC+0x051 ; Bottom of string space
        .equ    LINEAT  ,   WRKSPC+0x053 ; Current line number
        .equ    BASTXT  ,   WRKSPC+0x055 ; Pointer to start of program
        .equ    BUFFER  ,   WRKSPC+0x058 ; Input buffer
        .equ    STACK   ,   WRKSPC+0x05D ; Initial stack
        .equ    CURPOS  ,   WRKSPC+0x0A2 ; <<<< Character position on line ** Top of Loader TEMPSTACK **
        .equ    LCRFLG  ,   WRKSPC+0x0A3 ; Locate/Create flag
        .equ    TYPE    ,   WRKSPC+0x0A4 ; Data type flag
        .equ    DATFLG  ,   WRKSPC+0x0A5 ; Literal statement flag
        .equ    LSTRAM  ,   WRKSPC+0x0A6 ; Last available RAM
        .equ    TMSTPT  ,   WRKSPC+0x0A8 ; Temporary string pointer
        .equ    TMSTPL  ,   WRKSPC+0x0AA ; Temporary string pool
        .equ    TMPSTR  ,   WRKSPC+0x0B6 ; Temporary string
        .equ    STRBOT  ,   WRKSPC+0x0BA ; Bottom of string space
        .equ    CUROPR  ,   WRKSPC+0x0BC ; Current operator in EVAL
        .equ    LOOPST  ,   WRKSPC+0x0BE ; First statement of loop
        .equ    DATLIN  ,   WRKSPC+0x0C0 ; Line of current DATA item
        .equ    FORFLG  ,   WRKSPC+0x0C2 ; "FOR" loop flag
        .equ    LSTBIN  ,   WRKSPC+0x0C3 ; Last byte entered
        .equ    READFG  ,   WRKSPC+0x0C4 ; Read/Input flag
        .equ    BRKLIN  ,   WRKSPC+0x0C5 ; Line of break
        .equ    NXTOPR  ,   WRKSPC+0x0C7 ; Next operator in EVAL
        .equ    ERRLIN  ,   WRKSPC+0x0C9 ; Line of error
        .equ    CONTAD  ,   WRKSPC+0x0CB ; Where to CONTinue
        .equ    PROGND  ,   WRKSPC+0x0CD ; End of program
        .equ    VAREND  ,   WRKSPC+0x0CF ; End of variables
        .equ    ARREND  ,   WRKSPC+0x0D1 ; End of arrays
        .equ    NXTDAT  ,   WRKSPC+0x0D3 ; Next data item
        .equ    FNRGNM  ,   WRKSPC+0x0D5 ; Name of FN argument
        .equ    FNARG   ,   WRKSPC+0x0D7 ; FN argument value
        .equ    FPREG   ,   WRKSPC+0x0DB ; Floating point register
        .equ    FPEXP   ,   FPREG+3     ; Floating point exponent
        .equ    SGNRES  ,   WRKSPC+0x0DF ; Sign of result
        .equ    PBUFF   ,   WRKSPC+0x0E0 ; Number print buffer
        .equ    MULVAL  ,   WRKSPC+0x0ED ; Multiplier
        .equ    PROGST  ,   WRKSPC+0x0F0 ; Start of program text area
        .equ    STLOOK  ,   WRKSPC+0x154 ; Start of memory test

; BASIC ERROR CODE VALUES

        .global  UFERR           ; User Function undefined (RSTnn) error
        .global  HXERR, OMERR    ; Hex/data error, out-of-memory error
        .global  BRKRET          ; Return-to-command-prompt entry

        .equ    NF      ,   0x00 ; NEXT without FOR
        .equ    SN      ,   0x02 ; Syntax error
        .equ    RG      ,   0x04 ; RETURN without GOSUB
        .equ    OD      ,   0x06 ; Out of DATA
        .equ    FC      ,   0x08 ; Function call error
        .equ    OV      ,   0x0A ; Overflow
        .equ    OM      ,   0x0C ; Out of memory
        .equ    UL      ,   0x0E ; Undefined line number
        .equ    BS      ,   0x10 ; Bad subscript
        .equ    DD      ,   0x12 ; Re-DIMensioned array
        .equ    DZ      ,   0x14 ; Division by zero (/0)
        .equ    ID      ,   0x16 ; Illegal direct
        .equ    TM      ,   0x18 ; Type miss-match
        .equ    OS      ,   0x1A ; Out of string space
        .equ    LS      ,   0x1C ; String too long
        .equ    ST      ,   0x1E ; String formula too complex
        .equ    CN      ,   0x20 ; Can't CONTinue
        .equ    UF      ,   0x22 ; UnDEFined user function FN
        .equ    MO      ,   0x24 ; Missing operand
        .equ    HX      ,   0x26 ; HEX error

; BASIC CODE COMMENCES (DrunkPC port: was ORG 0x0240 in original RC2014)

.section .text

.global COLD, CSTART, WARMST, WARM
COLD:   JP      CSTART          ; Cold-start entry
WARM:   JP      WARMST          ; Warm-start entry
...
; skip "Memory top?" prompt and the RAM-probe loop.
MSIZE:  LD      HL,_basic_ram_end
        JP      SETTOP
...
BFREE:  .asciz   " Bytes free\n"

SIGNON: .ascii   "Z80 BASIC Ver 4.7c\n"
        .asciz   "Copyright (C) 1978 by Microsoft\n"
...
.ascii   "ND"    ; 80h
        ...
        .byte    'D'+80H
        .ascii   "OKE"
        .byte    'S'+80H         ; SAVE (was MOKE slot)
        .ascii   "AVE"
        .byte    'L'+80H         ; LOAD (was MEEK slot)
        .ascii   "OAD"
        .byte    'L'+80H
        .ascii   "INES"
        .byte    'C'+80H
        ...
        .byte    'C'+80H
        .ascii   "LEAR"
        .byte    'H'+80H         ; HLOAD: keyword kept for token alignment
        .ascii   "LOAD"          ;        executor jumps to UFERR ("?UF Error")
        .byte    'N'+80H
        .ascii   "EW"
...
WORDTB: .word    PEND
        ...
        .word    LIST
        .word    CLEAR
        .word    UFERR           ; HLOAD slot - placeholder so token table aligns;
                                 ; executing HLOAD now prints "?UF Error"
        .word    NEW
...
; RESERVED WORD TOKEN VALUES
        ...
        .equ    ZEND    ,   0x080        ; END
        .equ    ZFOR    ,   0x081        ; FOR
        .equ    ZDATA   ,   0x083        ; DATA
        .equ    ZGOTO   ,   0x088        ; GOTO
        .equ    ZGOSUB  ,   0x08C        ; GOSUB
        .equ    ZREM    ,   0x08E        ; REM
        .equ    ZPRINT  ,   0x09E        ; PRINT
        .equ    ZNEW    ,   0x0A3        ; NEW
        ...
        .equ    ZFN     ,   0x0A6        ; FN
        .equ    ZSPC    ,   0x0A7        ; SPC
        .equ    ZTHEN   ,   0x0A8        ; THEN
        .equ    ZNOT    ,   0x0A9        ; NOT
        .equ    ZSTEP   ,   0x0AA        ; STEP
...
INITAB: JP      WARMST          ; Warm start jump
        ...
        .byte    70              ; Width for commas (6 columns of 14 width)
        .byte    0               ; No nulls after input bytes
        .byte    0               ; Output enabled (^O off)

        .word    7               ; Initial lines counter (DrunkPC: 8-row display)
        .word    7               ; Initial lines number
        .word    0               ; Array load/save check sum

        .byte    0               ; Break not by NMI
        .byte    0               ; Break flag

        JP      TTYLIN_drunkpc  ; Input reflection: kutakbash line-editor glue

        .word    STLOOK          ; Temp string space
        .word    -2              ; Current line number (cold)
        .word    PROGST+1        ; Start of program text
INITBE:
        .space    3               ; Fill 3 Bytes for copy
...
ENFMEM: PUSH    HL              ; Save code string address
        LD      A,0x0D0          ; 48 Bytes minimum RAM
        SUB     L
        LD      L,A
        LD      A,0x0FF          ; 48 Bytes minimum RAM
        SBC     A,H
...
DINPOS: POP     AF              ; Restore character
        POP     BC              ; Restore buffer length
        JP      TXA             ; Tail-call OS putchar glue (was RST 08)

OUTNCR: CALL    OUTC            ; Output character in A
        JP      PRNTCRLF        ; Output CRLF

CLOTST: CALL    RXA              ; Get input character (was RST 0x10)
        AND     0b01111111       ; Strip bit 7
...
LIST:   CALL    ATOH            ; DE = start line (0 if absent)
        JR      Z,LISTNOEND     ; no '-' follows -> end = unlimited
        CP      ZMINUS          ; range separator '-' ?
        RET     NZ              ; garbage on the line -> silent bail
        PUSH    DE              ; save start while we parse end
        CALL    GETLN           ; DE = end (0 if nothing after '-').
                                ; GETLN skips ATOH's DEC HL, so the first
                                ; GETCHR advances past the '-' token.
        LD      A,D
        OR      E               ; was an end actually given?
        POP     HL              ; recover start into HL
        EX      DE,HL           ; DE = start, HL = end
        JR      NZ,LISTHASEND   ; explicit end given
LISTNOEND:
        LD      HL,0xFFFF       ; no upper limit -> list to end of program
LISTHASEND:
        LD      (LSTEND),HL     ; remember end for the per-line check below
...
LISTLP: POP     HL              ; Restore address of line
        ...
        INC     HL
        PUSH    HL              ; Save body addr around the compare
        LD      HL,(LSTEND)     ; Upper bound of the requested range
        OR      A               ; clear carry for sbc
        SBC     HL,DE           ; end - line_num; CY set if line > end
        POP     HL              ; Restore body addr (flags preserved)
        JR      NC,LSTINRNG     ; line within range -> proceed
        POP     BC              ; discard saved next-line addr (clean stack)
        JP      PRNTOK          ; past the range -> done listing
LSTINRNG:
...
COUNT:  PUSH    HL              ; Save code string address
        ...
        CALL    RXA              ; Get input character (was RST 0x10)
        CP      CTRLC           ; Is it control "C"?
...
TSTBRK: CALL    RXA_CHK          ; Check input status (was RST 0x18)
        OR      A               ; Check count Zero
        RET     Z               ; No key, go back
        CALL    RXA              ; Get the key into A (was RST 0x10)
        CP      ESC             ; Escape key?
        JP      Z,BRK           ; Yes, break
        CP      CTRLC           ; <Ctrl-C>
        JP      Z,BRK           ; Yes, break
...
STALL:  CALL    RXA              ; Wait for key (was RST 0x10)
...
NEXT1:  CALL    NZ,GETVAR       ; Get index address
        ...
        LD      H,B           ; Set code string to loop
        LD      L,C
...
RND1:   LD      DE,(FPREG)      ; Move FPREG to BCDE
        ...
        LD      (HL),0x80        ; Set saved signed bit to positive
        DEC     HL              ; Point to Exponent
        LD      B,(HL)          ; Get Exponent to BCDE
        LD      (HL),0x80        ; Makes Exponent 1
...
CLRSCRN:
        .byte    ESC
        .ascii   "[2J"
        .byte    0     ; VT100 Clear screen escape code
...
LSTEND: .skip   2               ; Upper bound for the LIST n-N range command
